...
def show_award_analysis():
    st.markdown("<h1 style='text-align:center; color:#FFD700;'>🏆 Award Analysis Dashboard</h1>", unsafe_allow_html=True)
    df = load_data()
    ...
    with kpi1:
        st.markdown(f"<div style='background-color:#A7C7E7; padding:15px; border-radius:10px; text-align:center;'><h4>Total Awards</h4><h2>{total_awards}</h2></div>", unsafe_allow_html=True)
    with kpi2:
        st.markdown(f"<div style='background-color:#F4C2C2; padding:15px; border-radius:10px; text-align:center;'><h4>Unique Award Titles</h4><h2>{unique_award_titles}</h2></div>", unsafe_allow_html=True)
    with kpi3:
        st.markdown(f"<div style='background-color:#90EE90; padding:15px; border-radius:10px; text-align:center;'><h4>Top Team</h4><h2>{top_team}</h2></div>", unsafe_allow_html=True)
    with kpi4:
        st.markdown(f"<div style='background-color:#FFFACD; padding:15px; border-radius:10px; text-align:center;'><h4>Most Common Award</h4><h2>{most_common_award}</h2></div>", unsafe_allow_html=True)
    ...
    fig1 = px.bar(
        top_awards, x='Award Title', y='Count',
        text='Count', color='Award Title',
    ...
    # =============================
    # 👥 2️⃣ Team vs Award Type — Treemap View
    # =============================
    ...
    # =============================
    # 🔥 3️⃣ Top Award-Winning Teams — Gradient
    # =============================
    ...
    fig3 = px.bar(
        leaderboard, x='Team name', y='Award Count',
        color='Award Count', color_continuous_scale='Sunsetdark',
        text='Award Count', title="🌟 Top 10 Teams by Total Awards"
    )
    ...
    fig_time = px.bar(
    timeline,
    x="New_Award_title", y="Award Count",
